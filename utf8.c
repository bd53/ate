...
int utf8_char_len(const char *s, int max_len)
{
        ...
        if (c < 0xC0)
                return 0;
        if ((c & 0xE0) == 0xC0)
                return (max_len >= 2) ? 2 : 0;
        if ((c & 0xF0) == 0xE0)
                return (max_len >= 3) ? 3 : 0;
        if ((c & 0xF8) == 0xF0)
                return (max_len >= 4) ? 4 : 0;
        return 0;
}

int utf8_decode(const char *s, int max_len, int *codepoint)
{
        if (s == NULL || codepoint == NULL || max_len <= 0)
                return -1;
        unsigned char c = (unsigned char) *s;
        if (c < 0x80) {
                *codepoint = c;
        ...
        if ((c & 0xF0) == 0xE0) {
                if (max_len < 3 || (s[1] & 0xC0) != 0x80
                    || (s[2] & 0xC0) != 0x80)
                        return -1;
                *codepoint =
                    ((c & 0x0F) << 12) | ((s[1] & 0x3F) << 6) | (s[2] & 0x3F);
                if (*codepoint < 0x800
        ...
        if ((c & 0xF8) == 0xF0) {
                if (max_len < 4 || (s[1] & 0xC0) != 0x80
                    || (s[2] & 0xC0) != 0x80 || (s[3] & 0xC0) != 0x80)
                        return -1;
                *codepoint =
                    ((c & 0x07) << 18) | ((s[1] & 0x3F) << 12) | ((s[2] & 0x3F)
                                                                  << 6) | (s[3]
                                                                           &
                                                                           0x3F);
...
int utf8_next_char_boundary(const char *s, int byte_offset, int max_len)
{
        ...
        if (!utf8_is_char_boundary(s, byte_offset)) {
                byte_offset++;
                while (byte_offset < max_len
                       && !utf8_is_char_boundary(s, byte_offset))
                        byte_offset++;
                return byte_offset;
        }
        ...
        if (len == 0)
                return (byte_offset + 1 > max_len) ? max_len : byte_offset + 1;
        byte_offset += len;
...
void insert_utf8_character(const char *utf8_char, int char_len)
{
        ...
        if (!utf8_is_char_boundary(row->chars, Editor.cursor_x))
                Editor.cursor_x =
                    utf8_prev_char_boundary(row->chars, Editor.cursor_x);
        int codepoint;
        ...
        if (!new_chars)
                die("realloc");
        row->chars = new_chars;
        memmove(&row->chars[Editor.cursor_x + char_len],
                &row->chars[Editor.cursor_x], row->size - Editor.cursor_x + 1);
        ...
        if (!new_hl)
                die("realloc");
        row->state = new_hl;
        memmove(&row->state[Editor.cursor_x + char_len],
                &row->state[Editor.cursor_x],
                row->size - Editor.cursor_x - char_len);
