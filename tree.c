...
static void scan_directory(const char *path, int depth)
{
        ...
        while ((entry = readdir(dir)) != NULL) {
                ...
                if (temp_count >= temp_capacity) {
                        temp_capacity =
                            temp_capacity == 0 ? 32 : temp_capacity * 2;
                        char **new_names =
                            realloc(temp_names, sizeof(char *) * temp_capacity);
                        char **new_paths =
                            realloc(temp_paths, sizeof(char *) * temp_capacity);
                        int *new_is_dir =
                            realloc(temp_is_dir, sizeof(int) * temp_capacity);
                        int *new_depth =
                            realloc(temp_depth, sizeof(int) * temp_capacity);
                        if (!new_names || !new_paths || !new_is_dir
                            || !new_depth) {
                                char **cleanup_names =
                                    new_names ? new_names : temp_names;
                                char **cleanup_paths =
                                    new_paths ? new_paths : temp_paths;
        ...
        if (temp_count > 0) {
                ...
                for (int i = 0; i < temp_count; i++) {
                        if (num_entries >= entries_capacity) {
                                entries_capacity =
                                    entries_capacity ==
                                    0 ? 64 : entries_capacity * 2;
                                char **new_names = realloc(file_names,
                                                           sizeof(char *) *
                                                           entries_capacity);
                                char **new_paths = realloc(file_paths,
                                                           sizeof(char *) *
                                                           entries_capacity);
                                int *new_is_dir = realloc(file_is_dir,
                                                          sizeof(int) *
                                                          entries_capacity);
                                int *new_depth = realloc(file_depth,
                                                         sizeof(int) *
                                                         entries_capacity);
                                if (!new_names || !new_paths || !new_is_dir
                                    || !new_depth) {
...
static void build_file_tree(const char *root_path)
{
        ...
        const char *scan_path =
            root_path ? root_path : getcwd(cwd, sizeof(cwd));
        if (!scan_path) {
...
void open_file_tree(void)
{
        ...
        if (Editor.cursor_y == 3 && current_row->size >= 2
            && strncmp(row_content, "../", 3) == 0) {
                char cwd[1024];
        ...
        if (Editor.cursor_y < 4)
                return;
        int entry_index = Editor.cursor_y - 4;
        if (entry_index < 0 || entry_index >= num_entries)
                return;
