...
{
        struct Row *row = (Editor.row != NULL && Editor.cursor_y >= 0
                           && Editor.cursor_y <
                           Editor.buffer_rows) ? &Editor.
            row[Editor.cursor_y] : NULL;
        Editor.found_row = -1;
        Editor.found_col = -1;
        switch (key) {
        case KEY_ARROW_LEFT:
                if (Editor.cursor_x > 0) {
                        ...
                        Editor.cursor_y--;
                        if (Editor.row != NULL && Editor.cursor_y >= 0
                            && Editor.cursor_y < Editor.buffer_rows)
                                Editor.cursor_x =
                                    Editor.row[Editor.cursor_y].size;
                }
                break;
        case KEY_ARROW_RIGHT:
                if (row != NULL && Editor.cursor_x < row->size) {
                        Editor.cursor_x++;
                } else if (row != NULL && Editor.cursor_x == row->size
        ...
        case KEY_CTRL_ARROW_DOWN:{
                        int jump =
                            (Editor.editor_rows / 7 <
                             1) ? 1 : Editor.editor_rows / 7;
                        Editor.cursor_y +=
                            (key == KEY_CTRL_ARROW_UP) ? -jump : jump;
                        if (Editor.cursor_y < 0)
                                Editor.cursor_y = 0;
        ...
        row = (Editor.row != NULL && Editor.buffer_rows > 0
               && Editor.cursor_y >= 0
               && Editor.cursor_y <
               Editor.buffer_rows) ? &Editor.row[Editor.cursor_y] : NULL;
