...
static int check_command(const char *cmd, char *output_buf, size_t buf_size,
                         const char *version_flag)
{
        ...
        if (version_flag && output_buf) {
                snprintf(command, sizeof(command), "%s %s 2>&1 | head -n 1",
                         cmd, version_flag);
                fp = popen(command, "r");
                if (fp) {
...
static void check_language_support(void)
{
        ...
        for (size_t i = 0; i < sizeof(languages) / sizeof(languages[0]); i++) {
                ...
                if (check_command(cmd, version, sizeof(version), flag)) {
                        ...
                        append_health_line(0, "WARNING %s: not available",
                                           name);
                        append_advice
                            ("spawn: %s failed with exit code - and signal -. Could not find executable \"%s\" in PATH.",
                             cmd, cmd);
...
static void check_os_info(void)
{
        ...
        if (uname(&sys_info) == 0) {
                ...
                snprintf(line, sizeof(line), "  machine = \"%s\",",
                         sys_info.machine);
                append_row(line, strlen(line));
                snprintf(line, sizeof(line), "  release = \"%s\",",
                         sys_info.release);
                append_row(line, strlen(line));
                snprintf(line, sizeof(line), "  sysname = \"%s\",",
                         sys_info.sysname);
                append_row(line, strlen(line));
        ...
        for (size_t i = 0; i < sizeof(tools) / sizeof(tools[0]); i++) {
                ...
                if (check_command(cmd, NULL, 0, NULL)) {
                        snprintf(command, sizeof(command),
                                 "command -v %s 2>/dev/null", cmd);
                        fp = popen(command, "r");
                        if (fp) {
                                if (fgets(path, sizeof(path), fp)) {
                                        path[strcspn(path, "\n")] = 0;
                                } else {
                                        strncpy(path, "unknown",
                                                sizeof(path) - 1);
                        ...
                        append_health_line(1, "OK %s: %s (%s)", name, version,
                                           path);
                } else {
...
static void check_clipboard(void)
{
        ...
        for (size_t i = 0;
             i < sizeof(clipboard_tools) / sizeof(clipboard_tools[0]); i++) {
                if (check_command(clipboard_tools[i], NULL, 0, NULL)) {
                        append_health_line(1, "OK Clipboard tool found: %s",
                                           clipboard_tools[i]);
                        found = 1;
                        break;
                }
