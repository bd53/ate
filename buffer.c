...
static void insert_row(int at, char *s, size_t len)
{
        if (at < 0 || at > Editor.buffer_rows)
                return;
        Editor.row =
            realloc(Editor.row, sizeof(struct Row) * (Editor.buffer_rows + 1));
        if (!Editor.row)
                die("realloc");
        memmove(&Editor.row[at + 1], &Editor.row[at],
                sizeof(struct Row) * (Editor.buffer_rows - at));
...
static void delete_row(int at)
{
        ...
        memmove(&Editor.row[at], &Editor.row[at + 1],
                sizeof(struct Row) * (Editor.buffer_rows - at - 1));
        if (--Editor.buffer_rows == 0) {
                ...
                Editor.row =
                    realloc(Editor.row,
                            sizeof(struct Row) * Editor.buffer_rows);
...
void insert_character(char c)
{
        ...
        if (Editor.cursor_x < old_size) {
                memmove(&row->chars[Editor.cursor_x + 1],
                        &row->chars[Editor.cursor_x],
                        old_size - Editor.cursor_x);
                memmove(&row->state[Editor.cursor_x + 1],
                        &row->state[Editor.cursor_x],
                        old_size - Editor.cursor_x);
        }
...
void insert_new_line(void)
{
        ...
        if (Editor.cursor_y >= 0 && Editor.cursor_y < Editor.buffer_rows) {
                ...
                for (int i = 0; i < Editor.row[Editor.cursor_y].size; i++) {
                        if (Editor.row[Editor.cursor_y].chars[i] == ' '
                            || Editor.row[Editor.cursor_y].chars[i] == '\t') {
                                indent++;
                        } else {
                                break;
                        }
                ...
                if (split < Editor.row[Editor.cursor_y].size) {
                        ...
                        if (!split_content)
                                die("malloc");
                        memcpy(split_content,
                               &Editor.row[Editor.cursor_y].chars[split],
                               split_len);
                        split_content[split_len] = '\0';
                        for (int i = 0; i < split_len; i++) {
                                if (split_content[i] != ' '
                                    && split_content[i] != '\t') {
                                        dedent = (split_content[i] == '}'
                                                  || split_content[i] == ')'
                                                  || split_content[i] == ']');
                                        break;
                                }
                ...
                if (split_content) {
                        insert_row(Editor.cursor_y + 1, split_content,
                                   split_len);
                        free(split_content);
                } else {
        ...
        Editor.cursor_x = 0;
        Editor.modified = 1;
        int final_indent = indent + (extra
                                     && !dedent ? INDENT_SIZE : 0) - (dedent
                                                                      && indent
                                                                      >=
                                                                      INDENT_SIZE
                                                                      ?
...
void auto_dedent(void)
{
        ...
        if (leading >= INDENT_SIZE) {
                ...
                if (spaces_to_remove == 0)
                        return;
                memmove(row->chars, row->chars + spaces_to_remove,
                        row->size - spaces_to_remove + 1);
...
void delete_character(void)
{
        ...
        if (Editor.cursor_x == 0) {
                ...
                unsigned char *new_state = realloc(prev_row->state,
                                                   prev_row->size +
                                                   current_row->size);
                if (!new_state)
                        die("realloc");
                prev_row->state = new_state;
                memcpy(&prev_row->chars[merge_at], current_row->chars,
                       current_row->size);
                memcpy(&prev_row->state[merge_at], current_row->state,
                       current_row->size);
                ...
                if (bytes_to_move > 0) {
                        memmove(&row->chars[Editor.cursor_x - 1],
                                &row->chars[Editor.cursor_x], bytes_to_move);
                        memmove(&row->state[Editor.cursor_x - 1],
                                &row->state[Editor.cursor_x], bytes_to_move);
                }
                row->size--;
                row->chars[row->size] = '\0';
...
{
        if (Editor.buffer_rows > 0 && Editor.cursor_y >= 0
...
void goto_line(void)
{
        ...
        while (*endptr && isspace((unsigned char) *endptr))
                endptr++;
        int valid = (*endptr == '\0' && line_number > 0
                     && line_number <= Editor.buffer_rows);
        free(command);
        if (valid) {
...
static int calc_gutter_width(void)
{
        ...
        int max =
            Editor.buffer_rows >
            Editor.editor_rows ? Editor.buffer_rows : Editor.editor_rows;
        return (max < 9 ? 1 : max < 99 ? 2 : max < 999 ? 3 : 4) + 1;
}

static void render_line(struct Buffer *ab, int filerow, int wrap_line,
                        int content_width)
{
        ...
        if (!Editor.file_tree) {
                int len =
                    snprintf(buf, sizeof(buf), "\x1b[38;5;244m%s%*d \x1b[m",
                             filerow == Editor.cursor_y ? "\x1b[32m" : "",
                             Editor.gutter_width - 1,
                             wrap_line == 0 ? filerow + 1 : 0);
                if (wrap_line > 0)
                        len =
                            snprintf(buf, sizeof(buf),
                                     "\x1b[38;5;244m%*s \x1b[m",
                                     Editor.gutter_width - 1, "");
                append(ab, buf, len);
        }
        ...
        if (Editor.file_tree && (filerow < 3 || filerow == Editor.cursor_y))
                append(ab, filerow < 3 ? "\x1b[34m" : "\x1b[7m",
                       filerow < 3 ? 5 : 4);
        int current_hl = 0;
        for (int i = start; i < end; i++) {
                int hl = row->state[i];
                if (!Editor.file_tree && Editor.query
                    && filerow == Editor.found_row) {
                        int match_col = Editor.found_col;
                        if (i >= match_col
                            && i < match_col + (int) strlen(Editor.query))
                                hl = 1;
                }
                if (hl != current_hl) {
                        current_hl = hl;
...
static void draw_content(struct Buffer *ab)
{
        ...
        for (int filerow = Editor.row_offset;
             ...
             filerow++) {
                if (!Editor.row)
                        break;
                int wrapped =
                    (Editor.row[filerow].size + content_width -
                     1) / content_width;
                if (wrapped == 0)
                        wrapped = 1;
                for (int wrap = 0;
        ...
        while (screen_row < Editor.editor_rows) {
                if (!Editor.file_tree) {
                        char buf[32];
                        snprintf(buf, sizeof(buf), "\x1b[38;5;244m%*s\x1b[m",
                                 Editor.gutter_width, "~");
...
static void handle_command(char *cmd)
{
        ...
        if (strcmp(trimmed_cmd, "q") == 0) {
                ...
                if (Editor.buffer_rows == 0)
                        append_row("", 0);
                Editor.cursor_x = Editor.cursor_y = Editor.row_offset =
                    Editor.modified = 0;
                refresh_screen();
                ...
                char msg[256];
                snprintf(msg, sizeof(msg), "E182: Not an editor command: '%s'",
                         trimmed_cmd);
...
void refresh_screen(void)
{
        ...
        char buf[64];
        snprintf(buf, sizeof(buf), "\x1b[%d;%dH%s\x1b[?25h", cur_y, cur_x,
                 Editor.mode == 1 ? "\x1b[5 q" : "\x1b[2 q");
        append(&ab, buf, strlen(buf));
        if (write(STDOUT_FILENO, ab.b, ab.length) == -1)
                die("write");
        free(ab.b);
