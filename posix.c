...
void ttopen(bool enable)
{
        if (enable) {
                ...
                raw.c_iflag &=
                    ~(IGNBRK | BRKINT | IGNPAR | PARMRK | INPCK | INLCR | IGNCR
                      | ICRNL | IXON);
                raw.c_oflag &=
                ...
                if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &Editor.original) == -1)
                        die("tcsetattr");
                for (size_t i = 0;
                     i < sizeof(exit_sequences) / sizeof(exit_sequences[0]);
                     ++i)
                        write(STDOUT_FILENO, exit_sequences[i],
                              strlen(exit_sequences[i]));
        }
}
