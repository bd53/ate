...
void display_status(struct Buffer *ab)
{
        ...
        if (Editor.file_tree) {
                ...
                snprintf(filename_status, sizeof(filename_status), "%s%s",
        ...
        int percentage =
            (Editor.buffer_rows >
             0) ? (int) (((float) (Editor.cursor_y + 1) / Editor.buffer_rows) *
                         100) : 100;
        int len =
            snprintf(status, sizeof(status), " %s | %s | R:%d L:%d (%d%%)",
                     mode_str, filename_status, Editor.cursor_y + 1,
                     Editor.buffer_rows > 0 ? Editor.buffer_rows : 1,
                     percentage);
        ...
        char rstatus[80];
        int rlen =
            snprintf(rstatus, sizeof(rstatus), " %s | C:%d ", filetype_name,
                     Editor.cursor_x + 1);
